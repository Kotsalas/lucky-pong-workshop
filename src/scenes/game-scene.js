...
export class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.ball = new Ball(this, width / 2, height / 2, 'tennis-ball');
    ...
    this.physics.add.collider(this.ball, this.leftPaddle, (ball, paddle) => {
        this.handleBounce(ball, paddle);
        this.events.emit('BALL_COLLISION');
    }, null, this);
    this.physics.add.collider(this.ball, this.rightPaddle, (ball, paddle) => {
        this.handleBounce(ball, paddle);
        this.events.emit('BALL_COLLISION');
    }, null, this);
  ...
  update() {
    ...
    // Score Check
    if (this.ball.x < 0) {
      this.scoring.increment('right');
      this.ball.reset();
    } else if (this.ball.x > this.scale.width) {
      this.scoring.increment('left');
      this.ball.reset();
    }
  }
